...
fn main() -> Result<(), String> {
    ...
    let mut scale = cli.scale.unwrap_or(1.0);
    let backend = cli.backend;

    let texture_creator: TextureCreator<WindowContext>;
    let left_svg = get_texture_builder(cli.files[0].to_owned(), backend)?;
    let right_svg = get_texture_builder(cli.files[1].to_owned(), backend)?;
